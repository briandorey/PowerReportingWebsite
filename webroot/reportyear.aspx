...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            SqlDataSource1.SelectCommand = "SELECT TOP (100) PERCENT AVG(watertop) AS watertop, AVG(waterbase) AS waterbase, AVG(waterpanel) AS waterpanel, AVG(hometemp) AS hometemp, DATEPART(yyyy, eDate) AS DateYear FROM dbo.HomeLog GROUP BY  DATEPART(yyyy, eDate)";
            Repeater1.DataBind();
            ...
            objConn.Open();
            System.Data.SqlClient.SqlDataAdapter daTax = new System.Data.SqlClient.SqlDataAdapter("SELECT TOP (100) PERCENT MAX(watertop) AS watertop, MAX(waterpanel) AS waterpanel, MAX(hometemp) AS hometemp, MAX(mainsc) AS mainsc, MAX(solarc) AS solarc, MAX(inverterc) AS inverterc, MAX(generalc) AS generalc, DATEPART(yyyy, eDate) AS DateYear  FROM dbo.HomeLog GROUP BY  DATEPART(yyyy, eDate)", objConn);
            daTax.FillSchema(dsTax, System.Data.SchemaType.Source, "HomeLog");
            daTax.Fill(dsTax, "HomeLog");
            daTax.Dispose();
            objConn.Close();

            SetMaxValues(dsTax.Tables[0].Rows[0]["watertop"].ToString(), dsTax.Tables[0].Rows[0]["waterpanel"].ToString(), dsTax.Tables[0].Rows[0]["hometemp"].ToString());
            
            SetMaxCurrent(dsTax.Tables[0].Rows[0]["mainsc"].ToString(), dsTax.Tables[0].Rows[0]["solarc"].ToString(), dsTax.Tables[0].Rows[0]["inverterc"].ToString(), dsTax.Tables[0].Rows[0]["generalc"].ToString());
            ...
            */
            // pv data
            SqlDataSourcePV.SelectCommand = "SELECT TOP (100) PERCENT AVG(mainsc) AS mainsc, AVG(solarc) AS solarc, AVG(inverterc) AS inverterc,AVG(generalc) AS generalc ,DATEPART(yyyy, eDate) AS DateYear FROM dbo.HomeLog   GROUP BY  DATEPART(yyyy, eDate)";
            Repeater2.DataBind();

            // battery data
            SqlDataSourceBattery.SelectCommand = "SELECT TOP (100) PERCENT AVG(batteryv) AS batteryv , DATEPART(yyyy, eDate) AS DateYear  FROM dbo.HomeLog  GROUP BY  DATEPART(yyyy, eDate)";
            Repeater2.DataBind();
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
 ...
 <script type="text/javascript" src="https://www.google.com/jsapi"></script>
  ...
  <ItemTemplate>
      ...
      ['<%# DataBinder.Eval(Container.DataItem, "Dateyear") %>', <%# DataBinder.Eval(Container.DataItem, "watertop") %>, <%# DataBinder.Eval(Container.DataItem, "waterpanel") %>, <%# DataBinder.Eval(Container.DataItem, "hometemp") %>],
  </ItemTemplate>
</asp:Repeater>
            ]);
  ...
  <ItemTemplate>
      ...
      ['<%# DataBinder.Eval(Container.DataItem, "Dateyear") %>', <%# DataBinder.Eval(Container.DataItem, "solarc") %>, <%# DataBinder.Eval(Container.DataItem, "inverterc") %>, <%# DataBinder.Eval(Container.DataItem, "generalc") %>, <%# DataBinder.Eval(Container.DataItem, "mainsc") %>],
  </ItemTemplate>
</asp:Repeater>
        ]);
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
 ...
 <div id="chart_div" style="width: 960px; height: 400px;"></div>
    ...
    <div id="chart_divVoltage" style="width: 960px; height: 400px; "></div>
