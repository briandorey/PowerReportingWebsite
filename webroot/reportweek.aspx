...
<script runat="server">
    ...
    public void setdata(DateTime dt){
        ...
        SqlDataSource1.SelectCommand = "SELECT TOP (100) PERCENT AVG(watertop) AS watertop, AVG(waterbase) AS waterbase, AVG(waterpanel) AS waterpanel, AVG(hometemp) AS hometemp, DATEPART(dd, eDate) AS DateDay, DATEPART(hour, dateadd(hour,(datediff(hour,0,eDate)/6)*6,0)) AS DateHour, pumprunning  FROM dbo.HomeLog WHERE eDate > '" + dt.AddDays(-7).ToString("yyyy-MM-dd") + "' AND eDate < '" + dtEnd.ToString("yyyy-MM-dd") + "'  GROUP BY dateadd(hour,(datediff(hour,0,eDate)/6)*6,0), DATEPART(dd, eDate), pumprunning order by DATEPART(dd, eDate) asc";
            Repeater1.DataBind();
            ...
            // pv data
            SqlDataSourcePV.SelectCommand = "SELECT TOP (100) PERCENT AVG(mainsc) AS mainsc, AVG(solarc) AS solarc, AVG(inverterc) AS inverterc,AVG(generalc) AS generalc ,DATEPART(dd, eDate) AS DateDay, DATEPART(hour, dateadd(hour,(datediff(hour,0,eDate)/6)*6,0)) AS DateHour FROM dbo.HomeLog  WHERE eDate > '" + dt.AddDays(-7).ToString("yyyy-MM-dd") + "' AND eDate < '" + dtEnd.ToString("yyyy-MM-dd") + "' GROUP BY dateadd(hour,(datediff(hour,0,eDate)/6)*6,0),  DATEPART(dd, eDate)";
            Repeater2.DataBind();

            // battery data
            SqlDataSourceBattery.SelectCommand = "SELECT TOP (100) PERCENT AVG(batteryv) AS batteryv , DATEPART(dd, eDate) AS DateDay, DATEPART(hour, dateadd(hour,(datediff(hour,0,eDate)/6)*6,0)) AS DateHour FROM dbo.HomeLog   WHERE eDate > '" + dt.AddDays(-7).ToString("yyyy-MM-dd") + "' AND eDate < '" + dtEnd.ToString("yyyy-MM-dd") + "' GROUP BY dateadd(hour,(datediff(hour,0,eDate)/6)*6,0),  DATEPART(dd, eDate)";
            Repeater2.DataBind();
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
 ...
 <script type="text/javascript" src="https://www.google.com/jsapi"></script>
  ...
  <ItemTemplate>
      ...
      ['<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateDay").ToString()) %>:<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateHour").ToString()) %>', <%# DataBinder.Eval(Container.DataItem, "watertop") %>, <%# DataBinder.Eval(Container.DataItem, "waterpanel") %>, <%# DataBinder.Eval(Container.DataItem, "hometemp") %>, <%# CheckPump(DataBinder.Eval(Container.DataItem, "pumprunning").ToString()) %>],
  ...
  <ItemTemplate>
      ...
      ['<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateDay").ToString()) %>:<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateHour").ToString()) %>', <%# DataBinder.Eval(Container.DataItem, "solarc") %>, <%# DataBinder.Eval(Container.DataItem, "inverterc") %>, <%# DataBinder.Eval(Container.DataItem, "generalc") %>, <%# DataBinder.Eval(Container.DataItem, "mainsc") %>],
  </ItemTemplate>
</asp:Repeater>
            ]);
