...
<script runat="server">
 string dateformat = "HH:mm";
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            string sql = "SELECT TOP 60 watertop AS datavalue, eDate AS DateField, 'x' AS DateField2 FROM dbo.HomeLog  order by eDate DESC";
           DateTime dt = DateTime.Now;
            if (Request.QueryString["o"] != null)
            {
                switch (Request.QueryString["o"].ToString())       
                  {         
                     case "day":
                          sql = "SELECT TOP (100) PERCENT AVG(watertop) AS datavalue, DATEPART(hh, eDate) AS DateField,  DATEPART(minute, dateadd(minute,(datediff(minute,0,eDate)/15)*15,0)) AS DateField2  FROM dbo.HomeLog WHERE  (DATEPART(dd, eDate) = " + dt.ToString("dd") + ") AND (DATEPART(mm, eDate) = " + dt.ToString("MM") + ") AND (DATEPART(yyyy, eDate) = " + dt.ToString("yyyy") + ") GROUP BY dateadd(minute,(datediff(minute,0,eDate)/15)*15,0), DATEPART(dw, eDate), DATEPART(hh, eDate), DATEPART(dd, eDate), DATEPART(mm, eDate), DATEPART(yyyy, eDate)";
                        break;
                     case "week":

                        sql = "SELECT TOP (100) PERCENT AVG(watertop) AS datavalue, DATEPART(dd, eDate) AS DateField,  DATEPART(hh, eDate) DateField2  FROM dbo.HomeLog WHERE  eDate >= '" + dt.AddDays(-7).ToString("s") + "' GROUP BY DATEPART(dw, eDate), DATEPART(hh, eDate), DATEPART(dd, eDate), DATEPART(mm, eDate), DATEPART(yyyy, eDate)";
                        break;
                     case "month":
                        sql = "SELECT TOP (100) PERCENT AVG(watertop) AS datavalue, DATEPART(mm, eDate) AS DateField,  DATEPART(dd, eDate) DateField2  FROM dbo.HomeLog WHERE  eDate >= '" + dt.AddMonths(-1).ToString("s") + "' GROUP BY DATEPART(dw, eDate), DATEPART(dd, eDate), DATEPART(mm, eDate), DATEPART(yyyy, eDate)";
                        break;
                     case "year":
                        sql = "SELECT TOP (100) PERCENT AVG(watertop) AS datavalue, DATEPART(yyyy, eDate) AS DateField,  DATEPART(mm, eDate) DateField2  FROM dbo.HomeLog WHERE  eDate >= '" + dt.AddMonths(-1).ToString("s") + "' GROUP BY DATEPART(dw, eDate), DATEPART(mm, eDate), DATEPART(yyyy, eDate)";
                        break;          
                     default:
                        break;
                  }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
            <a href="tempcyltop.aspx?o=hour" class="<%= checkactive("hour") %>">Hour</a>
            <a href="tempcyltop.aspx?o=day" class="<%= checkactive("day") %>">Day</a>
            <a href="tempcyltop.aspx?o=week" class="<%= checkactive("week") %>">Week</a>
            <a href="tempcyltop.aspx?o=month" class="<%= checkactive("month") %>">Month</a>
            <a href="tempcyltop.aspx?o=year" class="<%= checkactive("year") %>">Year</a>
