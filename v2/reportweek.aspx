...
<script runat="server">
    ...
    public void setdata(DateTime dt){
        ...
        SqlDataSource1.SelectCommand = "SELECT TOP (100) PERCENT AVG(watertop) AS watertop, AVG(waterbase) AS waterbase, AVG(waterpanel) AS waterpanel, AVG(TempLivingRoom) AS TempLivingRoom, DATEPART(dd, eDate) AS DateDay, DATEPART(hour, dateadd(hour,(datediff(hour,0,eDate)/6)*6,0)) AS DateHour, pumprunning  FROM dbo.HomeLogGeneral WHERE eDate > '" + dt.AddDays(-7).ToString("yyyy-MM-dd") + "' AND eDate < '" + dtEnd.ToString("yyyy-MM-dd") + "'  GROUP BY dateadd(hour,(datediff(hour,0,eDate)/6)*6,0), DATEPART(dd, eDate), pumprunning order by DATEPART(dd, eDate) asc";
            Repeater1.DataBind();
        ...
        SqlDataSourceMains.SelectCommand = "SELECT TOP (100) PERCENT AVG(avgmainsc) AS mainsc, MAX(maxmainsc) AS mainscmax, DATEPART(dd, EntryDate) AS DateDay, DATEPART(hour, dateadd(hour,(datediff(hour,0,EntryDate)/6)*6,0)) AS DateHour FROM dbo.MainsData   WHERE EntryDate > '" + dt.AddDays(-7).ToString("yyyy-MM-dd") + "' AND EntryDate < '" + dtEnd.ToString("yyyy-MM-dd") + "' GROUP BY dateadd(hour,(datediff(hour,0,EntryDate)/6)*6,0),  DATEPART(dd, EntryDate)";
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
 ...
 <script type="text/javascript" src="https://www.google.com/jsapi"></script>
  ...
  <ItemTemplate>
      ...
      ['<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateDay").ToString()) %>:<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateHour").ToString()) %>', <%# DataBinder.Eval(Container.DataItem, "watertop") %>, <%# DataBinder.Eval(Container.DataItem, "waterpanel") %>, <%# DataBinder.Eval(Container.DataItem, "TempLivingRoom") %>, <%# CheckPump(DataBinder.Eval(Container.DataItem, "pumprunning").ToString()) %>],
  ...
  <ItemTemplate>
      ...
      ['<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateDay").ToString()) %>:<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateHour").ToString()) %>', <%# DataBinder.Eval(Container.DataItem, "batteryv") %>],
         ...
         ['Hour', 'Avg Amps', 'Max Amps'],
         <asp:Repeater ID="RepeaterMains" runat="server" DataSourceID="SqlDataSourceMains">
  <ItemTemplate>
     
      ['<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateDay").ToString()) %>:<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateHour").ToString()) %>', <%# DataBinder.Eval(Container.DataItem, "mainsc") %>, <%# DataBinder.Eval(Container.DataItem, "mainscmax") %>],
  </ItemTemplate>
</asp:Repeater>
            ]);
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <h1><a href="/default.aspx"><i class="fa fa-bar-chart" aria-hidden="true"></i> Data for <asp:Literal ID="Literal1" runat="server"></asp:Literal></a></h1>
