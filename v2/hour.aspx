...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            SqlDataSource1.SelectCommand = "SELECT pumprunning, AVG(watertop) AS watertop, AVG(waterbase) AS waterbase, AVG(waterpanel) AS waterpanel, AVG(TempLivingRoom) AS hometemp, AVG(TempBedroomTwo) AS TempBedroomTwo, AVG(TempBedroomOne) AS TempBedroomOne, eDate FROM dbo.HomeLogGeneral   WHERE  eDate >= '" + dt.ToString("s") + "' GROUP BY eDate, pumprunning order by eDate DESC";
            ...
            // mains data
              SqlDataSourceMains.SelectCommand = "SELECT avgmainsc, maxmainsc ,EntryDate FROM dbo.MainsData   WHERE  EntryDate >= '" + dt.ToString("s") + "' order by EntryDate ASC";
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
 ...
 <script type="text/javascript" src="https://www.google.com/jsapi"></script>
  ...
  <ItemTemplate>
      ...
      ['<%# FormatDate(DataBinder.Eval(Container.DataItem, "EntryDate").ToString()) %>', <%# DataBinder.Eval(Container.DataItem, "avgmainsc") %>, <%# DataBinder.Eval(Container.DataItem, "maxmainsc") %>],
  </ItemTemplate>
</asp:Repeater>
            ]);
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <h1><a href="/default.aspx"><i class="fa fa-clock-o" aria-hidden="true"></i> Last Hour</a></h1>
